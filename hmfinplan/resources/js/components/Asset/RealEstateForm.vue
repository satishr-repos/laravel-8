<template>
<div class="container">
    <inline-form ref="InlineForm" title="Real Estate Details" 
        ...
        <div slot="form_fields">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    <label class="input-label" for="desc">Address</label>
                    <input class="input" id="desc" type="text" placeholder="title" v-model="realEstate.desc">
                </div>
                
                <div>
                    <label class="input-label" for="area">Area</label>
                    <input class="input" id="area" type="text" placeholder="title" v-model="realEstate.area">
                </div>

                <div>
                    <label class="input-label" for="purchyr">Purchase Date</label>
                    <input class="input" id="purchyr" type="date" placeholder="" v-model="realEstate.purchase_yr">
                </div>
            
                <div>
                    <label class="input-label" for="purchcst">Purchase Cost</label>
                    <input class="input" id="purchcst" type="number" v-model="realEstate.purchase_cost">
                </div>
                
                <div>
                    <label class="input-label" for="growthrt">Expected Growth Rate</label>
                    <input class="input" id="growthrt" type="number" step="0.05" v-model="realEstate.expct_growth_rt">
                </div>

                <div>
                    <label class="input-label" for="currentval">Current Value</label>
                    <input class="input" id="currentval" type="number" step="0.05" v-model="realEstate.current_val">
                </div>

                <form-select label="Status" 
                    :selection.sync="realEstate.status"
                    :options="[ 
                        { value: 1, text:'Active'}, 
                        { value: 0, text:'Sold'} ]">
                </form-select>
...
export default {
    ...
    created() {
        Object.assign(this.realEstate, this.formData);
    },
    ...
    methods: {
        ...
        formSubmitted() {
            ...
            {
                axios.post(this.baseRoute, this.realEstate)
                    .then((response) => {
                ...
                axios.patch(route, this.realEstate)
                    .then((response) => {
